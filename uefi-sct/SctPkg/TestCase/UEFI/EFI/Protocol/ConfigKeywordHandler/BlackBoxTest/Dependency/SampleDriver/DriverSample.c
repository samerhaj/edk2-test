...
for (Index = 0; Index < StrLen(Str); Index++) {
  DEBUG ((DEBUG_INFO|DEBUG_LOAD, "%c", *(Str+Index)));
}
DEBUG ((DEBUG_INFO|DEBUG_LOAD, "\n-----------------------------------------\n"));
  ...
  )
{
  ...
  //
  Status = gBS->LocateProtocol (&gBlackBoxEfiConfigKeywordHandlerProtocolGuid, NULL, (VOID **) &HiiKeywordHandler);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  HiiHandle[0] = HiiAddPackages (
                   &gDriverSampleFormSetGuid,
                   DriverHandle[0],
                   ConfigKeywordHandler_DriverSampleStrings,
                   VfrBin,
                   NULL
                   );
  if (HiiHandle[0] == NULL) {
...
//while(1);
  ...
  //KeywordString = "x-UEFI-ns&PATH=01041400f4274aa000df424db55239511302113d7fff0400&KEYWORD=BIOSVendor&VALUE=AcmeBIOSCompany&x-UEFI-extension-ACME&KEYWORD=SpecialSettingX&VALUE=3";

  //NAMESPACE=x-UEFI-ns&PATH=01041400f4274aa000df424db55239511302113d7fff0400&KEYWORD=iSCSIBootEnable&VALUE=01

//
// Test Case 2: Try to set the readonly question value
//
